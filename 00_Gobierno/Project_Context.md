...
- **Data Quality / DAT:** `R-DAT-001` y `R-DAT-002` están materializadas sobre una única cadena factual `DataSnapshotFreshnessProducer → DataSnapshotFreshnessObservation + Evidence`. Ambas consumen la misma `ResolvedConfiguration(P-DAT-001) + Evidence`; DAT001 evalúa “dentro del horizonte” y DAT002 “supera el horizonte”, con complementariedad solo cuando ambas son `EVALUABLE`. Fecha futura, ausencia o contradicción → `NOT_EVALUABLE`. `R-DAT-003` permanece bloqueada: antigüedad no equivale a insuficiencia y DAT001/DAT002 no sustituyen QTG.
