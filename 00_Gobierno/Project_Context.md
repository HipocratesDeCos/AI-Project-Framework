...
- **Profitability / MGE:** `MGE-AUTH v0.1`, `Profitability Core v0.1` y su frontera `ProvenancedProfitabilityExecution` están materializados y validados. El core calcula margen únicamente desde `AuthorizedSaleBasis + AuthorizedCostBasis`, sin selección de fuentes ni Rules. La ejecución `R-MGE-*` permanece bloqueada: MGE-AUTH v0.1 no concedió autoridad de ejecución de Rules; `R-MGE-002` carece además de transformación matemática cerrada para `P-MGE-003`, y los valores iniciales `P-MGE-001/002/003` no pueden hardcodearse. P-MGE-004/005/006 continúan fuera de consumo.
