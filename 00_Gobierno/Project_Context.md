...
## Estado de continuidad reconciliado — 22/09/2026
...
- **Payment / PAG:** `R-PAG-001` y `R-PAG-002` disponen de relaciones parámetro → regla documentadas. El carrier factual canónico de plazo ofrecido ya está materializado: `SupplierEvidenceResult → PaymentTermSemanticAuthority → PaymentTermObservationAdapter → OfferedPaymentTermObservation`, con semántica explícita, `unit=days`, sin conversiones y multiplicidad fail-closed. Esto cierra el gap de escalar cuando existe una observación PAYMENT_TERM explícita y autorizada. la transformación exacta de `P-PAG-003` ya está autorizada y materializada como `effective_threshold = P-PAG-002 - P-PAG-003`, con configuración/evidencia coherentes y fail-closed. `P-PAG-004` ya está autorizado y materializado para `R-PAG-001`: `Sí → ENABLED`, `No → DISABLED`, y DISABLED implica exclusión voluntaria del criterio, nunca FALSE. `P-PAG-005` queda reconciliado y materializado como control independiente de contexto económico: no contiene el valor del descuento y no es prerequisito de evaluabilidad del comparador base. `R-PAG-001` ya está materializada como core ejecutable provenance-safe: reconstruye en la misma invocación el control `P-PAG-004`, el carrier de plazo ofrecido y el threshold `P-PAG-002 - P-PAG-003`, exige coherencia completa de identidad/configuración/evidencia y evalúa estrictamente `offered < effective_threshold`; igualdad o superioridad → FALSE. Está integrada en `run_domain_rules` y catálogo con metadata `R2 / ALTA / NEGOCIAR`. La normalización multicuota sigue fuera de alcance cuando falta el escalar explícito. `R-PAG-002` está materializada e integrada end-to-end en alcance **pago único**. El core provenance-safe reconstruye `P-PAG-004`, plazo ofrecido, `P-PAG-001`, baseline Finance Basic y el escenario minimum-term; `PAG002-CF-DUE-DATE` está cerrado para un único PAYMENT explícitamente vinculado y genera exclusivamente una hipótesis `SCENARIO_ONLY` mediante `AuthorizedScenarioChange`, sin mutar el `CashFlow` factual. La clasificación financiera baseline/counterfactual reutiliza `P-FIN-002`; `TRUE → COMPRAR CONDICIONADO` con metadata `R1 / ALTA`. Multicuota, días fraccionarios o pertenencia del pago no demostrable permanecen `NOT_EVALUABLE` y fuera del alcance v0.1. `P-PAG-005` continúa como contexto económico opcional no bloqueante.
- **Historical / HIS:** `R-HIS-001`, `R-HIS-002` y `R-HIS-003` están materializadas en su alcance autorizado. `R-HIS-001` consume `HistoricalReferenceTemporalObservation + Evidence` vinculadas a la `PurchaseOperation` exacta y `ResolvedConfiguration(P-DAT-002) + Evidence`, con meses calendario, clipping y fail-closed. `R-HIS-003` reconstruye dentro del bridge una `HistoricalCommercialComparabilityObservation` desde la `PriceReference` exacta y siete determinaciones dimensionales gobernadas (`QUANTITY`, `SUPPLIER`, `COMMERCIAL_CONDITIONS`, `DISCOUNTS`, `RAPPELS`, `PAYMENT_TERM`, `ARTICLE_CHARACTERISTICS`). Cualquier `MATERIALLY_DIFFERENT` autorizado activa `TRUE`; todas `EQUIVALENT/NOT_APPLICABLE` producen `FALSE`; ausencia de autoridad/evidencia suficiente en una dimensión mantiene `NOT_DETERMINABLE → NOT_EVALUABLE`. No se promociona automáticamente la comparabilidad primaria de Price Intelligence ni se inventan umbrales, scoring o productores empresariales dimensionales.
...
- **Supplier Alternatives / PROV:** `R-PROV-001` y `R-PROV-002` están materializadas e integradas como core ejecutable provenance-safe. Ambas reconstruyen `SupplierEvidenceResult` desde el `SupplierEvidenceInput` exacto y consumen carriers PROV explícitos, gobernados y vinculados por evidencia. `R-PROV-001 TRUE` exige una candidatura `EVIDENCED_CANDIDATE` con `POTENTIALLY_BETTER`; `FALSE` exige `COMPLETE` y prueba negativa exhaustiva. `R-PROV-002 TRUE` exige sobre el mismo candidato `EVIDENCED_CANDIDATE + COMPARABLE + SIGNIFICANT_IMPROVEMENT`; `FALSE` exige `COMPLETE`, universo resuelto y ausencia exhaustiva de esa combinación. Metadata cerrada: `R-PROV-001 → R2 / MEDIA / NEGOCIAR`; `R-PROV-002 → R2 / ALTA / NEGOCIAR`. PR #305, CI #1158 y CI post-merge #1159 SUCCESS. Permanecen sin productor empresarial genérico autorizado las determinaciones `POTENTIALLY_BETTER`, `COMPARABLE` y `SIGNIFICANT_IMPROVEMENT`; Supplier Evidence Core continúa factual y no se autorizan ranking, scoring, ganador, proveedor recomendado ni cambio automático.
