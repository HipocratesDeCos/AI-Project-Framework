...
- **Profitability / MGE:** `MGE-AUTH v0.1`, `Profitability Core v0.1` y su frontera `ProvenancedProfitabilityExecution` están materializados y validados. El core calcula margen únicamente desde `AuthorizedSaleBasis + AuthorizedCostBasis`, sin selección de fuentes ni Rules. La ejecución `R-MGE-001/002/003` ha recibido autorización humana explícita mediante `MGE-RULES-AUTH v0.1`. La semántica cerrada es: `R-MGE-001: m < minimum`; `R-MGE-002: m >= minimum AND m >= target - tolerance AND m < target`; `R-MGE-003: m >= target`. Las tres reglas deben consumir un único bundle coherente de `P-MGE-001/002/003` resueltas y evidenciadas; sus valores iniciales no pueden hardcodearse y no se autoriza escalada R0. P-MGE-004/005/006 continúan fuera de consumo.
