...
**HEAD técnico auditado:** `285066b94e479e749d794742fb96303afbc4a2ee`  
...
- tests específicos Stage 2, Assessment→Scenario, VF→Scenario, E2E Scenario, frontera Decision Twin provenance y namespace público de `eios.rules`.
...
## 5. Test legado Assessment→Scenario reconciliado

Una CI posterior falló en colección porque `tests/test_assessment_scenario_integration.py` seguía importando los tres símbolos Stage 2 retirados desde `eios.rules` y probaba la antigua ruta positiva mediante `ViabilityResult` construido por el test.

Se ha reconciliado el archivo para que:

- exija que los tres símbolos Stage 2 no estén presentes en `rules.__all__` ni como atributos del namespace;
- exija que `eios.rules.scenario_integration.__all__` esté vacío;
- preserve y compruebe explícitamente `AssessmentTraceBinding` y `validate_assessment_trace_binding` como frontera C0 provenance-safe;
- no use el helper core interno para reconstruir artificialmente la antigua garantía pública.

La pasada preventiva posterior confirmó además que los tests vecinos de C0 provenance, Scenario Coordination, presentación y orquestación vertical usan sus fronteras core propias y no necesitan modificación.
...
## 10. Tests de conformidad depurados

Se elimina de los tests de Stage 2, Assessment→Scenario, E2E y Decision Twin integration la construcción manual de `ViabilityResult` usada para afirmar procedencia pública.
...
- los tests del antiguo límite público verifican la cuarentena en vez de recrear la ruta retirada;
...
La corrección elimina la contradicción detectada, sus consumidores aguas abajo y las expectativas/test legados desactualizados sin desplazar el problema, sin inventar infraestructura ausente y sin reabrir componentes core cerrados.
