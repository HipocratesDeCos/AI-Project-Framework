# EIOS — SUPPLIER EVIDENCE CORE · IMPLEMENTATION CONTRACT v0.3.1

**Estado:** DEPURADO — PENDIENTE DE AUDIT 2 FINAL  
**Fecha:** 11/09/2026  
**Autoridad metodológica:** Supplier Evidence Core v0.3 🔒 CERRADO  
**Sustituye:** `Supplier_Evidence_Core_Implementation_Contract_v0.3.md`
...
## 16. StructuralComparisonRequest
...
`comparison_authority_ref` solo referencia una autoridad externa ya resuelta; Supplier no la crea ni interpreta como regla.

---

## 17. StructuralComparisonResult
...
## 21. Reglas estrictas de tipos/unidades

La implementación valida después del parseo:

- value_integer es int real y no bool;
- value_boolean es bool real;
- exactamente un value_* en KNOWN;
- value_decimal es Decimal finito;
...
## 22. Algoritmo determinista de comparación estructural
...
Si existe conflicto, se preservan issue_refs.
...
Para DECIMAL/INTEGER se exige igualdad exacta de unit. DATE/TEXT/BOOLEAN ya deben tener unit=None.
...
Solo cuando **ambas observaciones comparten dimension = PRICE_REFERENCE** y han superado los pasos anteriores:

si comparison_authority_ref es None:
...
Agregados de refs/issues/unresolved/conflicting/limitations se deduplican en orden de primera aparición.
...
7. agregar unresolved/conflicting establemente;
8. no mutar inputs.
...
18. bool/int estrictos;
19. DATE/TEXT/BOOLEAN con unit rechazado;
20. disponibilidad TEXT no se convierte;
21. historical fact futuro/captured futuro rechazado;
22. signal futuro/captured futuro rechazado;
23. proveedor ajeno en fact/metric/signal rechazado;
...
29. request explícito únicamente;
30. no cross-product;
...
35. dimensiones incompatibles → NOT_STRUCTURALLY_COMPARABLE sin exigir PRICE authority;
36. ambas PRICE sin authority ref → UNKNOWN;
37. ambas PRICE con authority ref no recalculan PRICE;
38. semantic/unit/value_kind incompatibles → NOT_STRUCTURALLY_COMPARABLE;
39. decimal compatible → diferencia exacta;
40. diferencia no genera mejor/peor;
41. context/purchase mismatch rechazado;
42. internal refs inválidas rechazadas;
43. orden estable;
44. no mutación;
45. resultado sin campos decisionales.
...
- compatibilidad C0;
- compatibilidad con metodología v0.3;
- 0 duplicación de autoridad PRICE/Q&T/Rules/CRC;
