...
from .models import ComparabilityStatus, EconomicBasisAssessment, EconomicBasisStatus, PriceIntelligenceInput, PriceReference, PriceReferenceAssessment, TemporalStatus
...
def assess_economic_basis(payload: PriceIntelligenceInput,reference: PriceReference,assessment: PriceReferenceAssessment):
    ...
    if basis is None:return assessment.model_copy(update={"economic_basis":EconomicBasisAssessment(),"normalization_status":"PENDING","limitation_refs":assessment.limitation_refs+("NORMALIZATION_BASIS_MISSING",)})
    unit:EconomicBasisStatus="RESOLVED" if reference.unit==basis.target_unit else "PENDING";currency:EconomicBasisStatus="RESOLVED" if reference.currency==payload.purchase_operation.currency else "PENDING"
    eb=EconomicBasisAssessment(unit=unit,currency=currency);limits=assessment.limitation_refs
...
def select_references(assessments: Sequence[PriceReferenceAssessment])->tuple[str,...]:
    """Select only references satisfying closed upstream gates; price-independent and deterministic."""
