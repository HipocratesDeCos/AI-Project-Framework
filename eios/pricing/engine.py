...
from .models import (ComparabilityStatus, EconomicBasisAssessment, EconomicBasisEvidence, EconomicDimension, PriceCounts, PriceIntelligenceAssessmentContext, PriceIntelligenceInput, PriceIntelligenceResult, PriceReference, PriceReferenceAssessment, TemporalStatus)
...
def run_price_intelligence(payload: PriceIntelligenceInput, context: PriceIntelligenceAssessmentContext) -> PriceIntelligenceResult:
    identified=identify_references(payload); unique=deduplicate_references(identified); assessments={a.reference_id:a for a in assess_comparability(payload,unique)}
    for rid,ref in unique:
        assessments[rid]=normalize_reference(payload,ref,assessments[rid]); temporal=context.temporal.get(rid)
        assessments[rid]=assess_temporality(assessments[rid],temporal_rule_reference=temporal[1],eligible=temporal[0]=="ELIGIBLE") if temporal else assess_temporality(assessments[rid])
    ...
    ordered=tuple(assessments[rid] for rid,_ in unique); selected=select_references(ordered); values=tuple(assessments[rid].normalized_unit_price for rid in selected if assessments[rid].normalized_unit_price is not None)
    sufficiency_status=assess_sufficiency(len(selected),context.sufficiency); pr_value,aggregation_method=aggregate_selected_prices(values) if selected else (None,"MEDIAN_UNWEIGHTED")
    n_comparable=sum(a.comparability=="COMPARABLE" for a in ordered); n_representative=sum(a.comparability=="COMPARABLE" and a.representativeness=="REPRESENTATIVE" for a in ordered)
    limitations=list(dict.fromkeys(x for a in ordered for x in a.limitation_refs)); limitations.extend(x for x in context.sufficiency.methodological_limitations if x not in limitations)
    traces=list(dict.fromkeys(x for a in ordered for x in (a.temporal_rule_reference,) if x)); traces.extend(x for x in (context.sufficiency.evidence_refs+(context.sufficiency.rule_reference,context.sufficiency.trace_reference)) if x and x not in traces)
    return PriceIntelligenceResult(decision_id=payload.decision_context.decision_id,scenario_id=payload.decision_context.scenario_id,data_snapshot_id=payload.decision_context.data_snapshot_id,methodology_version=payload.methodology_version,pr_value=pr_value,currency=payload.purchase_operation.currency if pr_value is not None else None,sufficiency_status=sufficiency_status,pr_status={"SUFFICIENT":"PR_AVAILABLE","LIMITED":"PR_LIMITED","NOT_JUSTIFIABLE":"PR_NOT_JUSTIFIABLE"}[sufficiency_status],pr_limitations=tuple(limitations),reference_set=selected,counts=PriceCounts(n_raw=len(identified),n_unique=len(unique),n_comparable=n_comparable,n_representative=n_representative,n_selected=len(selected)),aggregation_method=aggregation_method,trace_references=tuple(traces))
