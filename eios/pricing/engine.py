...
def run_price_intelligence(payload:PriceIntelligenceInput,context:PriceIntelligenceAssessmentContext)->PriceIntelligenceResult:
    ...
    ordered=tuple(assessments[rid] for rid,_ in unique);selected=select_references(ordered);values=tuple(assessments[rid].normalized_unit_price for rid in selected if assessments[rid].normalized_unit_price is not None);status=assess_sufficiency(len(selected),context.sufficiency);pr_value,method=aggregate_selected_prices(values) if selected else (None,"MEDIAN_UNWEIGHTED");limitations=list(dict.fromkeys(x for a in ordered for x in a.limitation_refs));limitations.extend(x for x in context.sufficiency.methodological_limitations if x not in limitations);traces=list(dict.fromkeys(x for a in ordered for x in (a.temporal_rule_reference,) if x));traces.extend(x for x in context.sufficiency.evidence_refs+(context.sufficiency.rule_reference,context.sufficiency.trace_reference) if x and x not in traces)
