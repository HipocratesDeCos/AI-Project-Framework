...
def create_reference_demo(output_dir: Path, *, with_price: bool = False,
    ...
    negative_run, negative_captures = execute_reference_business_case_with_selected_observations(
        ...
        with_negotiation_ladder=with_negotiation_ladder,
    )
    ...
    ladder_observations = pair("negotiation_ladder") if with_negotiation_ladder else None
    ...
    try:
        ...
        if ladder_observations is not None:
            for name, payload in zip(_LADDER_NAMES, ladder_observations):
                (stage / name).write_text(
                    json.dumps(payload, ensure_ascii=False, indent=2) + "\n",
                    encoding="utf-8",
                )
...
def verify_reference_demo(directory: Path) -> tuple[str, str]:
    ...
    ladder_present = tuple((directory / name).exists() for name in _LADDER_NAMES)
    if any(ladder_present) and not all(ladder_present):
        raise ValueError("Both NEGOTIATION_LADDER observations are required together")
    ladder_observations = (tuple(json.loads((directory / name).read_text(encoding="utf-8"))
                                 for name in _LADDER_NAMES) if all(ladder_present) else None)
    ...
    for variant, stored in (("negative", negative), ("qtg-eligible", eligible)):
        replayed, captures = execute_reference_business_case_with_selected_observations(
            ...
            with_negotiation_ladder=ladder_observations is not None,
        )
        ...
        for label, saved_pair, key in (
            ...
            ("NEGOTIATION_LADDER", ladder_observations, "negotiation_ladder"),
        ):
