...
def demonstrated_evidence() -> Evidence:
    return Evidence(
        evidence_id="E-001",
        source_type="ERP",
        source_ref="purchase-history/001",
        captured_at=date(2026, 8, 21),
        state="DEMONSTRATED",
        demonstration_ref="ERP:purchase-history/001",
    )
...
def gap_evidence() -> Evidence:
    return Evidence(
        ...
        source_type="ERP",
        source_ref="purchase-history/missing",
        captured_at=date(2026, 8, 21),
        state="GAP",
    )
...
def test_golden_positive_demonstrated_evidence_evaluates_true():
    assessments, traces = run_c0(
        ...
        [(make_rule(), lambda purchase, evidence: purchase.quantity > 0)],
    )
...
def test_golden_negative_missing_or_gap_evidence_is_not_evaluable():
    assessments, traces = run_c0(
        make_input(),
        make_context(),
        [gap_evidence()],
...
def test_absent_evidence_is_not_false():
    assessments, _ = run_c0(
        make_input(),
        make_context(),
        [],
        [(make_rule(), lambda purchase, evidence: False)],
    )
...
def test_rule_without_evidence_requirement_can_evaluate_false():
    ...
    assessments, _ = run_c0(
        ...
        make_context(),
        [],
        [(rule, lambda purchase, evidence: False)],
    )
...
def test_context_mismatch_is_rejected():
    ...
    with pytest.raises(ValueError, match="scenario_id"):
        run_c0(
            ...
            context,
            [demonstrated_evidence()],
            [(make_rule(), lambda purchase, evidence: True)],
        )
...
def test_rule_version_mismatch_is_rejected():
    ...
    with pytest.raises(ValueError, match="versión"):
        run_c0(
            ...
            make_context(),
            [demonstrated_evidence()],
            [(rule, lambda purchase, evidence: True)],
        )
