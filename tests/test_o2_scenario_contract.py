...
def test_scenario_ids_are_independent_of_purchase_context_scenario():
    context = DecisionContext(decision_id="D1", scenario_id="BASE", rules_version="R1", parameters_version="P1", data_snapshot_id="S1")
    purchase = PurchaseOperation(decision_id="D1", scenario_id="BASE", article_id="A1", supplier_id="S1", quantity=Decimal("1"), unit_price=Decimal("2"), currency="EUR", operation_date=date(2026, 9, 2))
    results = [O2ScenarioResult(scenario_id="ALT-A", status=O2ScenarioStatus.COMPLETED), O2ScenarioResult(scenario_id="ALT-B", status=O2ScenarioStatus.COMPLETED)]
...
def test_comparison_retains_incomplete_scenario_without_fabricating_value():
    complete = O2ScenarioResult(scenario_id="A", status=O2ScenarioStatus.COMPLETED, values={"tco": 100})
    incomplete = O2ScenarioResult(scenario_id="B", status=O2ScenarioStatus.PARTIALLY_COMPLETED, unresolved_items=("tco unavailable",))
    purchase = PurchaseOperation(decision_id="D1", scenario_id="BASE", article_id="A1", supplier_id="S1", quantity=Decimal("1"), unit_price=Decimal("2"), currency="EUR", operation_date=date(2026, 9, 2))
    context = DecisionContext(decision_id="D1", scenario_id="BASE", rules_version="R1", parameters_version="P1", data_snapshot_id="S1")
    package = build_support_package(purchase, context, [complete, incomplete])
    assert package.comparison is not None
